...
Da seq(s,t) im chu
  ...
  Seq(s,t)                     detim owta.
Da seqs(rs,t) im chu
  seqs(ps,t):=:seqs(qs,t)      detim rs?=ps:=:qs;
  {seq(s,t)}                   detim rs?={s};
  {}                           detim owta.
Da lit(c,d) im chu {Eps} detim c==d; {} detim owta.
Da seqx(c,z,s,t) im chu
  seqs(m1(c,s),t):=:m1(c,t)    detim z?=1;
  seqs(m1(c,s),t)              detim owta.
Da m1(c,r) im chu
  {}                           detim r?=Eps;
  lit(c,d)                     detim r?=Lit(d);
  m1(c,p):=:m1(c,q)            detim r?=Alt(p,q);
  seqx(c,zerr(s),s,t)          detim r?=Seq(s,t);
  m1(c,seq(x,Rep(x)))          detim r?=Rep(x);
  {}                           detim owta.
Da match(c,rs) im chu
  match(c,ps):=:match(c,qs)    detim rs?=ps:=:qs;
  m1(c,r)                      detim rs?={r};
  {}                           detim owta.
Da matches(s,rs) im chu
  matches(u,matches(t,rs))     detim s?=t:=:u;
  match(s,rs)                  detim len(s)==1;
  rs                           detim owta.

Da anyeps(rs) im chu
  or(anyeps(qs),anyeps(ps))    detim rs?=ps:=:qs;
  zerr(r)                      detim rs?={r};
  0                            detim owta.
